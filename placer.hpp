...
public:
	...
	std::shared_ptr<Patch> best_patch;
	    ...
	    std::cout << std::endl;
            std::cout << "flipped_ROut allocated：" << std::endl;
            for (int i = 0; i < 5; ++ i) {
                for (int j = 0; j < 5; ++ j) {
	    ...
	    std::cout << std::endl;
            std::cout << "flipped_ROut after plan：" << std::endl;
            for (int i = 0; i < 5; ++ i) {
                for (int j = 0; j < 5; ++ j) {
                    std::cout << "(" << flipped_ROut[i * dft_w + j][REAL]<< ","
                            << flipped_ROut[i * dft_w + j][IMAG] << ") ";
	...
	// nan shows up here	
	std::cout << std::endl;
        std::cout << "flipped_ROut right after dft：" << std::endl;
        for (int i = 0; i < 5; ++ i) {
	    for (int j = 0; j < 5; ++ j) {
		std::cout << "(" << flipped_ROut[i * dft_w + j][REAL]<< ","
                        << flipped_ROut[i * dft_w + j][IMAG] << ") ";
	...
	// TODO: there are nan in output_R, output_G, and output_B
		...
		if(std::isnan(flipped_RIn[i * dft_w + j][REAL])){
            		printf("nan in flipped_RIn\n");
        	}
            }
            std::cout << std::endl;
        }
